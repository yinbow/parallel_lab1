    ...
    while(update){
        ...
        for(int i = 0; i < data2.size(); i++){
            ...
            while(flags[i] && data1.find(head2[i]) != data1.end()){
                for(int k=0; k < head2[i] / 32 + 1; k+=4){
                    simd2 = vld1q_u32(&data2[i][k]);
                    simd1 = vld1q_u32(&data1[head2[i]][k]);
                    res = veorq_u32(simd1, simd2);
                    vst1q_u32(&data2[i][k], res);
                }
                //新的首项
...
void readData(
    ...
    while(iter != data1.end()){
        iter->second.resize(size + (4 - size % 4) , 0);
        iter++;
    }
    for(int i = 0; i < data2.size(); i++){
        data2[i].resize(size + (4 - size % 4) , 0);
    }
    for(int i = 0; i < data3.size(); i++){
        data3[i].resize(size, 0);
    }
